...
void main()
{
    ...
    for(int j=0; j<4; j++){
      ...
      for(float i=0.; i<maxIter; i++){
        ...
        if(length(z)>2.){
            //col = (col*float(j-1)+vec3(pow(cos(i/maxIter*10.*PI),2.), pow(cos(i/maxIter*10.*PI+PI/3.),2.), pow(cos(i/maxIter*10.*PI+2.*PI/3.),2.)))/i;
            col = (col*float(j-1)+vec3(pow(cos(i*log(maxIter)/1000.*PI),2.), 0, pow(cos(i*log(maxIter)/1000.*PI+2.*PI/3.),2.)))/float(j);
            break;
        }
      }
    }
